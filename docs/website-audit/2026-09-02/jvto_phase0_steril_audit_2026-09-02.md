...
### Revisi 3 — F5 / P3-WHYJVTO: dugaan "cache ISR basi" DIFALSIFIKASI

Ditulis pada sesi lanjutan 2026-09-02 setelah mutasi #1 dan #2 di-merge dan di-deploy, memakai jalur verifikasi yang §4 sendiri sebut tidak tersedia saat F5 ditulis ("tidak bisa saya buktikan tanpa memicu revalidate produksi, yang di luar batas non-mutasi tugas ini"). Revalidate itu akhirnya terjadi sebagai efek samping deploy mutasi #2, sehingga percobaan falsifikasi bisa dituntaskan.

- **Nilai lama**: `[dugaan, BELUM DIVERIFIKASI]` — live `/why-jvto/the-jvto-difference` menyajikan og:image yang tidak dihasilkan jalur kode manapun; diduga HTML sisa render kode lama, dan begitu revalidate berikutnya terjadi og:image untuk `/why-jvto/{slug}` bisa hilang tanpa ada commit penyebab.
- **Nilai baru**: dugaan itu **SALAH**. Penyebab kontradiksi adalah presedensi rute Next.js App Router, bukan kebasian cache. Tidak ada risiko og:image hilang pada revalidate.
- **Bukti yang mengubah**: dua jenis, saling independen.
  - `[terbaca]` Ketiga URL why-jvto yang disampel tidak dilayani `[slug]/page.tsx`. Masing-masing punya folder rute sendiri — `the-jvto-difference/page.tsx`, `our-story/page.tsx`, `community-standards/page.tsx` — dan ketiganya **sudah** memanggil `buildStaticRouteMetadata()` dengan override `openGraph` sendiri. Itulah sumber og:image yang terlihat live. Kode `[slug]/page.tsx` menyatakan presedensi ini secara eksplisit di baris 48-59 (`WHY_JVTO_FOLDER_ROUTED_SLUGS` = `our-story`, `the-jvto-difference`, `community-standards`, `our-team`, `reviews`), dan baris 67 memfilter kelima slug itu keluar dari `generateStaticParams()`. Filter yang sama diterapkan ke `route-output-index.json` menghasilkan **0 slug** — rute dinamis itu tidak melayani satu URL pun hari ini.
  - `[terukur]` Konfirmasi silang dari header: builder `buildStaticRouteMetadata()` menetapkan `openGraph.type = "article"` sebagai default (`jvto-web/src/lib/ecosystemContent/staticPageAdapter.ts:98`), sementara live menyajikan `og:type: website` — nilai itu datang dari override folder page, bukan dari builder.
  - `[terukur]` Uji revalidate yang findings CSV syaratkan ("force a revalidation and re-check og:image before closing") sudah dieksekusi. Urutan header pada `/why-jvto/the-jvto-difference`: `STALE` (sebelum merge) → `HIT` (setelah deploy jvto-web `9de8010a`) → **`MISS`** (setelah deploy ekosistem `a93f607c`, Actions run `33569668009`, yang berakhir dengan POST revalidasi 294 rute). `MISS` berarti cache benar-benar dibuang dan halaman dirender ulang dari kode saat ini. og:image tetap ada dan identik pada ketiga URL: `the-jvto-difference.webp`, `our-story.webp`, `default.jpg`.
- **Dampak turunan**:
  - Status P3 di §3.3 tidak berubah, tapi **klasifikasi mutasi #1 bergeser**: `5e06506c` bukan perbaikan cacat yang hidup di produksi. Ia menetapkan owner metadata pada rute yang saat ini melayani 0 URL, dan berlaku untuk slug why-jvto baru yang nanti diterbitkan dari ekosistem. Kriteria `handoff-003` ("metadata title/description/canonical all present; owner documented") tetap terpenuhi.
  - Entri §6 "Apakah commit deploy production benar-benar `485b2f85`/`ac722c4c`... Hipotesis F5 bergantung padanya" menjadi tidak relevan: hipotesisnya gugur terlepas dari jawaban pertanyaan itu.
  - Pelajaran metodologi, sejenis dengan Revisi 1: F5 membaca `[slug]/page.tsx` lalu menyampel URL yang dilayani file lain. Presedensi rute seharusnya dipetakan sebelum kontradiksi kode-vs-live dilaporkan — daftar file rute (`find why-jvto -name page.tsx`) lebih murah daripada hipotesis cache.
- **Batas klaim** (agar revisi ini tidak dibaca melebihi buktinya): falsifikasi berlaku untuk pola `/why-jvto/{slug}` dan ketiga slug folder-routed yang disampel. Revisi ini **tidak** menyentuh `/why-jvto/our-team/*` maupun `/why-jvto/reviews/*` (file rute berbeda), dan **tidak** memutakhirkan catatan §6 bahwa baseline `group_summary` menyebut 6 dari 17 halaman why-jvto missing og:image saat snapshot — angka itu belum dicek ulang.
